# This research for the network analysis focuses on the R implementation. The R installation and RStudio are need. You should also install the latest version of igraph for R:

# The igraph library provides versatile options for descriptive network analysis and visualization in R, Python, and C/C++
...
# 1. Reading network data from files

nodes <- read.csv("Dataset1-Media-Example-NODES.csv", header=T, as.is=T)

links <- read.csv("Dataset1-Media-Example-EDGES.csv", header=T, as.is=T)

# 2. Examine and prepare the data
...
## Check the nodes, edges, and their attributes with:
...
plot(net, edge.arrow.szie=0.4, vertex.label=NA)

# Plot 2 (by removing the loops in the graph)
...
## 5.1 Plot with curved edges (edge.curved=.1) and reduce arrow size:

plot(net, edge.arrow.size=.4, edge.curved=.1)

## 5.2 Set edge color to gray, and the node color to orange. 
...
## 5.3 The second way to set attributes is to add them to the igraph object. Let’s say we want to color our network nodes based on type of media, and size them based on audience size (larger audience -> larger node). We will also change the width of the edges based on their weight.
...
## 6.1 Density: The proportion of present edges from all possible edges in the network.

edge_density(net, loops=F)

ecount(net)/(vcount(net)*(vcount(net)-1)) #for a directed network

## 6.2 Reciprocity: The proportion of reciprocated ties (for a directed network).
...
## 6.4 Diameter: A network diameter is the longest geodesic distance (length of the shortest path between two nodes) in the network. In igraph, diameter() returns the distance, while get_diameter() returns the nodes along the first found path of that distance.
...
# Centrality functions (vertex level) and centralization functions (graph level). The centralization functions return res - vertex centrality, centralization and theoretical_max - maximum centralization score for a graph of that size.
# The centrality function can run on a subset of nodes (set with the vids parameter). This is helpful for large graphs where calculating all centralities may be a resource-intensive and time-consuming tasks.
...
# The hubs and authorities algorithm developed by Jon Kleinberg was initially used to examine web pages. Hubs were expected to contain catalogs with a large number of outgoing links; while authorities woud get many incoming links from hubs, presumably because of their high-quality relevant information.
...
# 8.1 Cliqeus: Find cliques (complete subgraphs of an undirected graph)
